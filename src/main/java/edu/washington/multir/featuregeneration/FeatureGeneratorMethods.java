...
import edu.washington.multir.featuregeneration.GeneralizedFeatureGenerator020314.DependencyType;
import edu.washington.multir.featuregeneration.GeneralizedFeatureGenerator020314.Direction;
...
//	}
	...
	public static List<Triple<CoreLabel, DependencyType, CoreLabel>> getDependencyPath(Integer arg1EndCharOffset,
		...
		recursiveDependencySearch(depPath,depPath,dependencyInfo,tokens,startTokenOffset,endTokenOffset,-1,0);
		return depPath;
	...
	private static void recursiveDependencySearch(
			...
			Integer previousStart, int count) {
		...
		if(startTokenOffset.equals(endTokenOffset)){
			...
			for(int i =0; i < dependencyInfo.size(); i++){
				...
				if(depTriple.first.equals(startTokenOffset)){
			...
			for(Triple<Integer,String,Integer> childTriple : children){
				...
				if(!childTriple.third.equals(previousStart)){
					recursiveDependencySearch(depPath,newCurrPath,dependencyInfo,tokens,childTriple.third,endTokenOffset,startTokenOffset,count+1);
				}
			}
			...
			if(parents.size() == 1){
				...
				Triple<CoreLabel,DependencyType,CoreLabel> parentPath = new Triple<>(parent,dt,child);
				List<Triple<CoreLabel,DependencyType,CoreLabel>> newCurrPath = new ArrayList<>();
				newCurrPath.addAll(currPath);
				newCurrPath.add(parentPath);
				if(!parentTriple.first.equals(previousStart)){
				 recursiveDependencySearch(depPath,newCurrPath,dependencyInfo,tokens,parentTriple.first,endTokenOffset,startTokenOffset,count+1);
				}
			}
			else{
				return;
